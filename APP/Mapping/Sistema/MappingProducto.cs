...
namespace Mapping.Sistema
{
    public class MappingProducto
    {
        ...
        {
            List<Entidades.Sistema.Producto> _Productos = new List<Entidades.Sistema.Producto>();
            Servicios.Conexion Conexion = new Servicios.Conexion();
            Conexion.Conectar("Sistema");
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    Entidades.Sistema.CategoriaProducto Categoria = MappingCategoriaProducto.ObtenerInstancia().RecuperarCategorias().Find(x => x.Nombre == dr["Nombre"].ToString());

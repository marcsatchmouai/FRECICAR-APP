...
namespace Mapping.Sistema
{
    public class MappingVenta
    {
        ...
        {
            List<Entidades.Sistema.Venta> _Ventas = new List<Entidades.Sistema.Venta>();
            Servicios.Conexion Conexion = new Servicios.Conexion();
            Conexion.Conectar("Sistema");
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    Entidades.Sistema.FormadeEnvio FormadeEnvio = MappingFormadeEnvio.ObtenerInstancia().RecuperarFormadeEnvio().Find(x => x.Nombre == dr["FormaEnvio"].ToString());
                    _Venta.Envio = FormadeEnvio;

                    Entidades.Sistema.FormadePago FormadePago = MappingFormadePago.ObtenerInstancia().RecuperarFormadePago().Find(x => x.Nombre == dr["FormaPago"].ToString());

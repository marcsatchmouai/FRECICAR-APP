...
namespace Mapping.Sistema
{
    public class MappingOrdenCompra
    {
        ...
        {
            List<Entidades.Sistema.OrdenCompra> _OrdenesCompras = new List<Entidades.Sistema.OrdenCompra>();
            Servicios.Conexion Conexion = new Servicios.Conexion();
            Conexion.Conectar("Sistema");
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    FormadeEnvio = MappingFormadeEnvio.ObtenerInstancia().RecuperarFormadeEnvio().Find(x => x.Nombre == dr["FormaEnvio"].ToString());
                    OrdenCompra.FormadeEnvio = FormadeEnvio;

                    Entidades.Sistema.FormadePago FormadePago;
                    FormadePago = MappingFormadePago.ObtenerInstancia().RecuperarFormadePago().Find(x => x.Nombre == dr["FormaPago"].ToString());
                    OrdenCompra.FormadePago = FormadePago;

                    Entidades.Sistema.Proveedor Proveedor;
                    Proveedor = MappingProveedor.ObtenerInstancia().RecuperarProveedor().Find(x => x.Cuit == Convert.ToInt64((dr["Proveedor"])));
                    ...
                    while (dr2.Read())
                    {
                        ...
                        DetalledeOrdenCompra.MateriaPrima = MappingMateriaPrima.ObtenerInstancia().RecuperarMateriasPrimas().Find(x => x.Codigo == Convert.ToInt32((dr2["MateriaPrima"])));
                        OrdenCompra.AgregarDetalle(DetalledeOrdenCompra);
                    }
                    _OrdenesCompras.Add(OrdenCompra);
                }
            }
            catch (SqlException Ex)
            {

...
namespace Mapping.Sistema
{
    public class MappingCuentaCorriente
    {
        ...
        {
            _coleccionCtasCtes = new List<Entidades.Sistema.CuentaCorriente>();
            RecuperarCuentasCorrientes();
        }

        public List<Entidades.Sistema.CuentaCorriente> ListarCuentasCorrientes()
        {
            return _coleccionCtasCtes;
        }

        private void RecuperarCuentasCorrientes()
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    while (dr2.Read())
                    {
                        ...
                        Entidades.Sistema.Pago oPago = MappingPago.ObtenerInstancia().Recuperar().Find(x =>x.Detalle == dr2["Detalle"].ToString());
                        ...
                        Entidades.Sistema.NotaCredito oNota = MappingNotaCredito.ObtenerInstancia().ListarNotas().Find(x => x.Detalle == dr2["Detalle"].ToString());
                    ...
                    _coleccionCtasCtes.Add(oCuentaCorriente);
                }
            }
